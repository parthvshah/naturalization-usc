...
from model.utils import load_disfluencies, load_corpus, START_TOKEN, END_TOKEN, save_model, load_model, PAD_TOKEN
...
def tokenize_and_pad(tokenizer, sentence, device):
    ...
    if seq_len > STRIDE:
        raise Exception("Found a sentence that was longer than the expected transformer stride!")
...
def autoregressive_predict(model, tokenizer, test_in_path, output_path, device):
    ...
    for line in test_corpus:
        ...
        for pos in range(len(split_line)):
            ...
            mask = torch.cat((torch.ones((pos+1)), torch.zeros((tokens.size(1)-(pos+1)))), dim=0).to(device)
            ...
            with torch.no_grad():
                ...
                working_sentence = working_sentence + " " + last_word_so_far + " " + next_selected_token
